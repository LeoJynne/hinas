...
function renew-caidan() {
    ...
    # GitHub 上脚本的 URL
    GITHUB_URL="https://raw.githubusercontent.com/LX-webo/hinas/main/caidan.sh"
    # 本地脚本的路径
    LOCAL_SCRIPT="/etc/caidan/caidan.sh"

    # 从 GitHub 下载脚本的最新版本
    LATEST_SCRIPT=$(curl -sSL $GITHUB_URL)

    # 检测到有更新时下载最新版本并覆盖本地文件
    if [[ $LATEST_SCRIPT != $(<$LOCAL_SCRIPT) ]]; then
        echo -e "${YELLOW}脚本已更新，正在下载最新版本...${NC}"
        # 使用curl下载最新版本的脚本并覆盖本地文件
        curl -sSL $GITHUB_URL -o $LOCAL_SCRIPT
        echo -e "${GREEN}最新版本已更新${NC}"
    else
        # 脚本已是最新版本
...
${NC}"
    ...
    echo -e "${YELLOW}10.tailscale穿透${NC}"
