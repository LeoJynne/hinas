...
${NC}"
    ...
    echo -e "${YELLOW}9. Cloudflare DNS解析${NC}"
    echo -e "${YELLOW}10.系统迁移${NC}"
    echo -e "${YELLOW}11.tailscale穿透${NC}"
    echo -e "${YELLOW}12.powershell公钥链接${NC}"
    echo -e "${RED}00.卸载脚本${NC}"
...
"
    ...
    7)
        ...
        while true; do
            ...
            echo -e "${YELLOW}12.安装ChatGPT${NC}"
            echo -e "${YELLOW}13.安装小雅Docker${NC}"
            ...
            function install_chatgpt() {
                ...
                # 图标
                wget -qN -P /var/www/html/img/png https://cdn.jsdelivr.net/gh/LX-webo/hinas@main/chatgpt.png
                cat <<EOF >/var/www/html/icons_wan/chatgpt.html
            <li>
                <a href="http://<?php echo \$lanip ?>:8181" target="_blank"><img class="shake" src="img/png/chatgpt.png" /><strong>ChatGPT</strong></a>
            </li>
EOF
                # 容器
                ...
                # 输入 license_id
                read -p "请前往 https://dash.pandoranext.com 获取并输入license_id: " license_id
...
EOF
                ...
                echo -e "${GREEN}安装完成，web地址: $IP:8181${NC}"
            }
...
文件夹ID：         |   浏览器地址复制                                             |${NC}"
                ...
                1)
                    ...
                    if [[ $confirmation == "y" ]]; then
                        ...
                        echo -e "${GREEN}安装完成，web地址: $IP:6789${NC}"
...
EOF
                ...
                4)
                    #清理阿里云缓存
                    bash -c "$(curl -s https://cdn.jsdelivr.net/gh/LX-webo/hinas@main/aliyun_clear.sh | tail -n +2)" -s 5
                    echo "按任意键继续..."
                    read -n 1 -s -r -p ""
                    ;;
                q | Q)
            ...
            function Lucky() {
                ...
                # 图标
                wget -qN -P /var/www/html/img/png https://cdn.jsdelivr.net/gh/LX-webo/hinas@main/lucky.png
...
EOF
            ...
            function UptimeKuma() {
                ...
                # 图标
                wget -qN -P /var/www/html/img/png https://cdn.jsdelivr.net/gh/LX-webo/hinas@main/UptimeKuma.png
                cat <<EOF >/var/www/html/icons_lan/UptimeKuma.html
            <li>
                <a href="http://<?php echo \$lanip ?>:3001" target="_blank"><img class="shake" src="img/png/UptimeKuma.png" /><strong>Uptime Kuma</strong></a>
            </li>
EOF
    ...
    8)
        ...
        # 图标
        wget -qN -P /var/www/html/img/png https://cdn.jsdelivr.net/gh/LX-webo/hinas@main/cockpit.png
...
EOF
    ...
    9)
        ...
        while true; do
            ...
            echo -e "${YELLOW}1.运行脚本${NC}"
            echo -e "${YELLOW}2.添加/更新定时任务${NC}"
            ...
            1)
                # 运行
                file="Cloudflare_ddns.sh"

                # 检查
                if [ ! -f "$file" ]; then
                    # 文件不存在，执行下载
                    wget -q https://cdn.jsdelivr.net/gh/LX-webo/hinas@main/"$file"
                else
                    # 文件已存在，不输出
                    wget -q --spider https://cdn.jsdelivr.net/gh/LX-webo/hinas@main/"$file"
                fi
            ...
            2)
                ...
                # 清理旧有的定时同步任务
                cron_task="/root/Cloudflare_ddns.sh"
                crontab -l | sed -e "\@$cron_task@d" | crontab -

                # 添加新的定时同步任务
                cron_task="*/5 * * * *  bash /root/Cloudflare_ddns.sh"
                crontab -l | {
                    cat
                    echo "$cron_task"
                } | crontab -
                echo -e "${GREEN}已添加定时任务：每5分钟运行一次脚本${NC}"

                # 日志清空

                # 清理旧有的定时同步任务
                cron_task="/root/cloudflare_ddns.log"
                crontab -l | sed -e "\@$cron_task@d" | crontab -

                # 添加新的定时同步任务
                cron_task="0 * * * * > /root/cloudflare_ddns.log"
                crontab -l | {
                    cat
                    echo "$cron_task"
                } | crontab -
                echo -e "${GREEN}已添加定时任务：每小时清空一次日志文件${NC}"

                ;;
            3)
                # 查看日志
                cat cloudflare_ddns.log
                ;;
            q | Q)
                break
                ;;
    ...
    12)
        ...
        echo -e "${YELLOW}请在 cmd 中输入 ssh-keygen 命令生成密钥，按以下格式创建并配置config文件${NC}"
        echo "Host 你想要的主机名"
        echo "HostName 服务器的IP地址"
        echo "User 服务器用户名"
        echo "IdentityFile 保存私钥文件的完整路径 例如：C:\Users\user\.ssh\id_rsa"

        #  提示输入
        read -p "请输入你的公钥: " user_key

        # 输入保存到 authorized_keys
        echo "$user_key" >~/.ssh/authorized_keys

        echo -e "${GREEN}密钥配置已更新.${NC}"
        echo "按任意键继续..."
        read -n 1 -s -r -p ""
        ;;
    00)
