<?php
session_start();
// 检查数据库文件是否存在，如果不存在则跳转到注册页面
if (!file_exists('css/db/users.db')) {
...
<body>
	...
	<div id="tips" class="row">
          <label>忘记密码？ SSH 终端输入 reset-login</label>
	</div> 
                ...
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === XMLHttpRequest.DONE) {
                        if (xhr.status === 200) {
                            ...
                            // 根据后端返回的响应处理结果
                            if (response === 'success') {
                                tips.innerHTML = '<label style="color:#20FF80;">登录成功！</label>'
                                // 可以重定向至其他页面等操作
                                setTimeout(()=>{window.location = "home.php"},1000);
                            } else {
                                tips.innerHTML = '<label style="color:#C22000;">登录失败，请检查用户名和密码！</label>';
                            }
                        } else {
                            tips.innerHTML = '<label style="color:#C22000;">请求失败，请稍后重试！</label>';
                        }
                    }
                };
                xhr.send(formData);
